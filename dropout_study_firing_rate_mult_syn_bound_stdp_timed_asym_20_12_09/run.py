...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    for rp_idx in range(repeats):
        ...
        burst_t = np.arange(0, 4 * int(2e-3 / S.DT), int(2e-3 / S.DT))
...
def quick_plot(m, repeats=1, show_connectivity=True, n_show_only=None, add_noise=True, dropouts=[{'E': 0, 'I': 0}]):
    ...
    for idx_r, rsps in enumerate(all_rsps):
        for idx_do, rsp_for_dropout in enumerate(rsps):
            ...
            driving_spk_times = activation_times[:, activation_times[1, :] == 0][0, :]

            midpoint = int(len(driving_spk_times) / 2)
            
            plot_params = {
                'early': driving_spk_times[:3],
                'late': driving_spk_times[-3:],
                'middle': driving_spk_times[midpoint:midpoint + 20]
            }
            ...
            for title, trigger_times in plot_params.items():
                for t_idx, trigger_time in enumerate(trigger_times):
                    ...
                    t_window = (trigger_time * S.DT, trigger_time * S.DT + 40e-3)
...
m2.GAMMA = 0.
m2.W_E_I_R = .5e-5
m2.W_I_E_R = 0.06e-5 * m2.N_EXC
m2.T_R_E = 2e-3
m2.W_MAX = 0.26 * 0.004 * 0.4
m2.M = 5
