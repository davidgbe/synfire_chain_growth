        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            for i_e in range(epochs):
                ...
                if i_e == 0:
                    ...
                    total_potentiation = 0.1 * (3e-2 * fr_update + 1e-3 * stdp_burst_pair + 0 * fr_pop_update)
...
title = f'single_btdp_ff_{clip(m2.W_INITIAL / (0.26 * 0.004))}_pf_{clip(m2.CON_PROB_FF, 2)}_pr_{clip(m2.CON_PROB_R, 2)}_eir_{clip(m2.W_E_I_R * 1e5)}_ier_{clip(m2.W_I_E_R * 1e5)}'
