...
M = Generic(
    ...
    I_SINGLE_FR_TRIALS=(1001, 1051),
    POP_FR_TRIALS=(2001, 2021),
...
S = Generic(RNG_SEED=args.rng_seed[0], DT=0.2e-3, T=180e-3, EPOCHS=20)
...
for i in range(1):
    ...
    if args.load_mat is not None and args.load_mat[0] is not '':
        w_r_e, w_r_i = load_weight_matrices(args.load_mat[0], n)
    print(w_r_e, w_r_i)

    all_rsps = quick_plot(M, run_title=title + f'_{n * 10}', w_r_e=w_r_e, w_r_i=w_r_i, dropouts=[
        {'E': 0.0, 'I': 0},
    ])