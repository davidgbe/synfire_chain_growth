...
S = Generic(RNG_SEED=args.rng_seed[0], DT=0.22e-3, T=550e-3, EPOCHS=3000)
...
def generate_exc_ff_chain(m): 
    ...
    def ff_unit_func(layer_idx, syn_prop, all_syn_props):
        ...
        for i, l_idx in enumerate(reversed(range(layer_idx))):
            ...
            strong_weight_gaussian = gaussian((m.PROJECTION_NUM,), w, 0.2 * w)
            weak_weight_guassian = 0.2 * w * np.random.exponential(scale=4, size=(m.PROJECTION_NUM,))
    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            for i_e in range(epochs):
                ...
                fig = plt.figure(figsize=(9 * scale, 27 * scale), tight_layout=True)
                ...
                graph_weight_matrix(w_e_e_r_copy, 'w_e_e_r\n', ax=axs[4], cmap='gist_stern')
                graph_weight_matrix(w_r_copy['E'][-m.N_INH:, :m.N_EXC], 'w_e_i\n', ax=axs[5], cmap='gist_stern')
                ...
                if i_e == 0:
                    ...
                    update_i = 0
                    w_e_e_r_sums = np.sum(w_r['E'][m.N_EXC:, :m.N_EXC], axis=1)

                    if surviving_cell_indices is not None:
                        i_weight_diff = (w_e_e_r_sums - np.sum(w_r_copy['E'][m.N_EXC:, :m.N_EXC], axis=1)) / w_e_e_r_sums
                        update_i = i_weight_diff.reshape(i_weight_diff.shape[0], 1)



                    e_total_potentiation = m.ETA * (m.ALPHA_1 * fr_update_e + m.BETA * stdp_burst_pair + m.GAMMA * fr_pop_step)
                    i_total_potentiation = m.ETA * (m.ALPHA_2 * update_i)
