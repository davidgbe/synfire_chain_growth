...
M.CON_PROBS_FF = M.CON_PROB_FF_SCALE * np.exp(-1 * (np.arange(M.FF_DEGREE) + 1) / M.CON_PROB_FF_CONST)
...
def generate_exc_ff_chain(m):
    ...
    def ff_unit_func(connection_prob):
        w = m.W_E_E_R / m.PROJECTION_NUM
        return gaussian_if_under_val(connection_prob, (m.PROJECTION_NUM, m.PROJECTION_NUM), w, w)

    ff_degrees = np.arange(m.FF_DEGREE) + 1
    unit_funcs = []
    for i, ff in enumerate(ff_degrees):
        con_prob_ff = m.CON_PROBS_FF[i]
        unit_funcs.append(partial(ff_unit_func, connection_prob=con_prob_ff))

    return generate_ff_chain(m.N_EXC, m.PROJECTION_NUM, unit_funcs, ff_deg=ff_degrees, tempering=np.ones(len(ff_degrees)))
