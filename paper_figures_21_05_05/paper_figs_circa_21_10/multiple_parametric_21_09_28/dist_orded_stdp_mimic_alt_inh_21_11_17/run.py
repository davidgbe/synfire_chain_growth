...
parser.add_argument('--inh_add_rate', metavar='a2', type=float, nargs=1)
...
M = Generic(
    ...
    INH_ADD_RATE=args.inh_add_rate[0],
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            for i_e in range(epochs):
                ...
                if i_e == 0:
                    ...
                    if surviving_cell_indices is not None:
                        i_weight_diff = np.sum(w_r['E'][m.N_EXC:, :m.N_EXC], axis=1) - np.sum(w_r_copy['E'][m.N_EXC:, :m.N_EXC], axis=1)
                        update_i = gaussian_if_under_val(m.INH_ADD_RATE, (m.N_INH, m.N_EXC), m.W_E_I_R, 0.2 * m.W_E_I_R)
                        update_i[:, ~surviving_cell_indices] = 0
                        update_i[(i_weight_diff < 0), :] = 0
                    ...
                    w_r_copy['E'][(m.N_EXC + m.N_SILENT):, :(m.N_EXC + m.N_SILENT)] += update_i
