...
M = Generic(
    ...
    W_E_I_R=0.8e-5,
    W_E_I_R_MAX=1.8e-5,
    W_E_I_R_SUMMED_MAX=int(0.05 * 400) * 1.8e-5,
    W_I_E_R=0.9e-5,
    W_A=0,
    W_E_E_R=0.26 * 0.004 * 0.3,
...
M.W_U_E = M.W_E_E_R / M.PROJECTION_NUM * 20
...
def generate_exc_ff_chain(m): 
    ...
    def ff_unit_func(layer_idx):
        ...
        if layer_idx > 1:
            cons_for_cell[:, m.PROJECTION_NUM * (layer_idx - 2) : m.PROJECTION_NUM * (layer_idx - 1)] = gaussian_if_under_val(m.CON_PROB_FF, (m.PROJECTION_NUM,), w, 0.2 * w)
...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    w_u = {
        ...
        'E': np.block([
            [ w_u_e, np.zeros([m.N_DRIVING_CELLS, m.N_INH]) ],
            [ np.zeros([m.N_EXC - m.N_DRIVING_CELLS, m.N_DRIVING_CELLS + m.N_INH]) ],
            [ np.zeros([m.N_SILENT, m.N_DRIVING_CELLS + m.N_INH]) ],
            [ np.zeros([m.N_INH, m.N_DRIVING_CELLS]), 2 * m.W_E_I_R * np.diag(np.ones(m.N_INH)) ],
        ]),

        'I': np.zeros((m.N_EXC + m.N_SILENT + m.N_INH , m.N_DRIVING_CELLS + m.N_INH)),

        'A': np.zeros((m.N_EXC + m.N_SILENT + m.N_INH, m.N_DRIVING_CELLS + m.N_INH)),
    }
    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            for i_e in range(epochs):
                ...
                ## inp spks
                spks_u_base = np.zeros((len(t), m.N_DRIVING_CELLS + m.N_INH), dtype=int)
                ...
                for t_idx, driving_cell_idx in zip(*activation_times.nonzero()):
                    ...
                    try:
                        ...
                        pass
                # l = int((m.INPUT_DELAY - 5e-3 + 15e-3) / S.DT) - int((m.INPUT_DELAY - 5e-3) / S.DT)
                # spks_u[int((m.INPUT_DELAY - 5e-3) / S.DT) : int((m.INPUT_DELAY - 5e-3 + 15e-3) / S.DT), -m.N_INH:] = np.random.poisson(S.DT * 1000, (l, m.N_INH))
