...
M = Generic(
    ...
    W_E_E_R_MAX=0.26 * 0.004 * 10 * 0.55,
...
S = Generic(RNG_SEED=args.rng_seed[0], DT=0.22e-3, T=180e-3, EPOCHS=1000)
np.random.seed(S.RNG_SEED)

M.CON_PROBS_FF = np.exp(-1 * np.arange(M.N_EXC / M.PROJECTION_NUM) / M.CON_PROB_FF_CONST)

M.W_U_E = M.W_E_E_R / M.PROJECTION_NUM * 10
    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            for i_e in range(epochs):
                ...
                axs[0].set_xlim(m.INPUT_DELAY * 1000, 180)
