...
M = Generic(
    ...
    W_E_I_R=2.5e-5,
    W_E_I_R_MAX=2.5e-5,
    W_E_I_R_SUMMED_MAX=21 * 2.5e-5,
    W_I_E_R=1.1e-5,
    W_A=0,
    W_E_E_R=0.26 * 0.004 * 0.55,
    W_E_E_R_MAX=0.26 * 0.004 * 2.8 * 0.55,
    W_MIN=1e-8,
...
def generate_exc_ff_chain(m): 
    ...
    def ff_unit_func(layer_idx):
        w = m.W_E_E_R_MAX / m.PROJECTION_NUM
        n_layers = int(m.N_EXC / m.PROJECTION_NUM)

        cons_for_cell = np.zeros((1, m.N_EXC))

        if layer_idx > 0:
            cons_for_cell[:, m.PROJECTION_NUM * (layer_idx - 1) : m.PROJECTION_NUM * layer_idx] = m.W_MIN * exponential_if_under_val(1, (m.PROJECTION_NUM,), 0.25)
            cons_for_cell[:, m.PROJECTION_NUM * (layer_idx - 1) : m.PROJECTION_NUM * layer_idx] += w * rand_per_row_mat(int(m.CON_PROB_FF * m.PROJECTION_NUM), (1, m.PROJECTION_NUM))
...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    if w_r_e is None:
        ...
        e_i_r = m.W_MIN * exponential_if_under_val(0.05, (m.N_INH, m.N_EXC), 0.25)
        e_i_r += m.W_E_I_R_MAX * rand_per_row_mat(int(m.N_EXC * m.E_I_CON_PROB), (m.N_INH, m.N_EXC))
