...
M = Generic(
    ...
    E_SINGLE_FR_TRIALS=(1, 101),
    I_SINGLE_FR_TRIALS=(301, 351),
    POP_FR_TRIALS=(601, 651),
...
S = Generic(RNG_SEED=args.rng_seed[0], DT=0.5e-3, T=180e-3, EPOCHS=9000)
...
M.KERNEL_PAIR = np.where(np.arange(M.CUT_IDX_TAU_PAIR) * S.DT > M.STDP_MIN_LAG, M.KERNEL_PAIR, 0)
            ...
            for i_e in range(epochs):
                ...
                if i_e == 0:
                    ...
                    if i_e >= m.E_SINGLE_FR_TRIALS[0] and i_e < m.E_SINGLE_FR_TRIALS[1]:
                        ...
                        print('e_cell_fr_setpoints', e_cell_fr_setpoints)
                        print(np.sum(spks_for_e_cells > 0, axis=0))
                        ...
                        if not m.SINGLE_CELL_FR_SYM:
                            e_diffs[e_diffs >= 0] = 0
                        scaled_e_diffs = e_diffs
                        fr_update_e = scaled_e_diffs.reshape(scaled_e_diffs.shape[0], 1) * np.ones((m.N_EXC + m.N_SILENT, m.N_EXC + m.N_SILENT)).astype(float)
