...
M = Generic(
    ...
    T_R_E=0.7e-3,  # refractory period (s)
    ...
    W_I_E_R=0.9e-5,
    W_A=2.5e-6,
    W_E_E_R=0.26 * 0.004 * 0.5,
    W_E_E_R_MAX=0.26 * 0.004 * 10 *  0.5,
    W_MIN=1e-8,
...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    if w_r_e is None:
        ...
        e_i_r += m.W_E_I_R * mat_1_if_under_val(m.E_I_CON_PROB, (m.N_INH, m.N_EXC))
    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            initial_summed_ei_weights = None
            ...
            for i_e in range(epochs):
                ...
                if i_e == 0:
                    initial_summed_ei_weights = w_r_copy['E'][(m.N_EXC + m.N_SILENT):, :(m.N_EXC + m.N_SILENT)].sum(axis=1)
                    ...
                    where_summed_max_exceeded = summed_i_cell_input > initial_summed_ei_weights
                    w_r_copy['E'][(m.N_EXC + m.N_SILENT):, :(m.N_EXC + m.N_SILENT)][where_summed_max_exceeded, :] *= (initial_summed_ei_weights[where_summed_max_exceeded] / summed_i_cell_input[where_summed_max_exceeded]).reshape(np.count_nonzero(where_summed_max_exceeded), 1)
