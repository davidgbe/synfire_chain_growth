...
M = Generic(
    ...
    E_E=0, E_I=-.09, E_A=None, T_E=.004, T_I=.004, T_A=.01,
...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    if w_r_i is None:
        ...
        t_e_r = np.concatenate([10 * m.W_I_E_R * np.ones((m.N_EXC - 3 * m.PROJECTION_NUM, m.N_TERMINAL)), np.zeros((3 * m.PROJECTION_NUM, m.N_TERMINAL))], axis=0)

        w_r_i = np.block([
            [ np.zeros((m.N_EXC, m.N_EXC)), i_e_r,  t_e_r ],
    ...
    pairwise_spk_delays = np.block([
        [int(2e-3 / S.DT) * np.ones((m.N_EXC, m.N_EXC)), int(0.2e-3 / S.DT) * np.ones((m.N_EXC, m.N_INH + m.N_TERMINAL))],
        [int(0.2e-3 / S.DT) * np.ones((m.N_INH + m.N_TERMINAL, m.N_EXC + m.N_INH + m.N_TERMINAL))],
    ]).astype(int)
    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            for i_e in range(epochs):
                ...
                fig = plt.figure(figsize=(9 * scale, 33 * scale), tight_layout=True)
                axs = [fig.add_subplot(gs[:2]), fig.add_subplot(gs[2:4]), fig.add_subplot(gs[4]), fig.add_subplot(gs[5]), fig.add_subplot(gs[6]), fig.add_subplot(gs[7:9]), fig.add_subplot(gs[9:])]
                ...
                axs[3].plot(summed_w_bins, summed_w_counts, color='red')
                if i_e > 0:
                    axs[3].plot(initial_summed_weights[0], initial_summed_weights[1], color='black', zorder=-1)
                ...
                axs[2].bar(spk_bins, freqs, alpha=0.5)
                axs[2].set_xlabel('Spks per neuron')
                ...
                axs[2].bar(spk_bins_i, freqs_i, color='black', alpha=0.5)

                exc_raster = np.stack(spks_for_e_cells.nonzero())
                first_spk_times = process_single_activation(exc_raster, m)

                def to_colors(counts):
                    n = 8
                    counts = copy(counts)
                    counts[counts >= n] = n - 1
                    color_palate = np.array(['black', 'purple', 'blue', 'green', 'yellow', 'orange', 'red', 'pink'])
                    colors_out = color_palate[counts]
                    return colors_out

                idx_to_freq_colors = to_colors(spks_for_e_cells.sum(axis=0))
                ...
                for j in range(2):
                    ...
                    axs[j].set_xlim(m.INPUT_DELAY * 1000, 300)
                ...
                if i_e > 0:
                    ...
                    def trim_diffs(diffs, setpoint_widths):
                        diffs[(diffs <= (setpoint_widths/2)) & (diffs >= (-1 * setpoint_widths/2))] = 0
                        diffs[diffs >= (setpoint_widths/2)] -= (setpoint_widths/2)
                        diffs[diffs <= (-1 * setpoint_widths/2)] += (setpoint_widths/2)
                        return diffs

                    # if i_e < m.E_SINGLE_FR_TRIALS[1] and m.SET_FR_FLAG:
                    #     e_diffs = e_cell_fr_setpoints - np.sum(spks_for_e_cells > 0, axis=0)
                    #     if m.SINGLE_CELL_LINE_ATTR == 1:
                    #         e_diffs = trim_diffs(e_diffs, 5)
...
for i in range(1):
    ...
    if args.load_run is not None and args.load_run[0] is not '':
        loaded_data = load_previous_run(os.path.join('./robustness', args.load_run[0]), 140)
