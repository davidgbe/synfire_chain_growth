...
M = Generic(
    ...
    W_I_E_R=0.1e-5,
    W_A=0,
    W_E_E_R=0.26 * 0.004 * 1.8,
    W_E_E_R_MAX=0.26 * 0.004 * 50 * 1.8,
...
S = Generic(RNG_SEED=args.rng_seed[0], DT=0.22e-3, T=250e-3, EPOCHS=2000)
np.random.seed(S.RNG_SEED)

M.CON_PROBS_FF = np.exp(-1 * np.arange(M.N_EXC / M.PROJECTION_NUM) / M.CON_PROB_FF_CONST)

M.W_U_E = M.W_E_E_R / M.PROJECTION_NUM * 15
