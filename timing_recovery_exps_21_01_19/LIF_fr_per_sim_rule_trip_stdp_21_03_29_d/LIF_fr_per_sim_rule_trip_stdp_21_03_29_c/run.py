        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            for i_e in range(epochs):
                ...
                if i_e == 0:
                    ...
                    e_cell_fr_setpoints = np.mean(np.sum(rsp.spks[:, :m.N_EXC] > 0, axis=0))
                ...
                if i_e == epochs - 1:
                    ...
                    spk_diff = (e_cell_fr_setpoints - np.sum(rsp.spks[:, :m.N_EXC] > 0, axis=0))
                    fr_update = spk_diff * np.abs(spk_diff)
                    ...
                    print(a * fr_update)
                    print(b * stdp_update)
                    print(a * fr_update + b * stdp_update - c)

                    w_r_e_update = (5e-7 * (a * fr_update + b * stdp_update - c) * np.where(w_r['E'][:m.N_EXC, :m.N_EXC] > 0, 1, 0))
