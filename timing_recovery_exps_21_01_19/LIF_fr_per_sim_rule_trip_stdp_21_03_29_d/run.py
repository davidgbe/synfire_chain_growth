...
parser.add_argument('--p_ff', metavar='P', type=float, nargs=1)
parser.add_argument('--p_r', metavar='S', type=float, nargs=1)
...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            e_cell_fr_setpoint_lower = None
            e_cell_fr_setpoint_upper = None
            i_cell_fr_setpoint = None

            for i_e in range(epochs):

                if i_e == 1:
                    w_r['E'][:, m.PROJECTION_NUM:m.N_EXC] = dropout_on_mat(w_r['E'][:, m.PROJECTION_NUM:m.N_EXC], dropout['E'])
                ...
                if i_e == 0:
                    ...
                    e_cell_fr_setpoint_lower = np.mean(np.sum(rsp.spks[:, :m.N_EXC] > 0, axis=0))
                    e_cell_fr_setpoint_upper = e_cell_fr_setpoint_lower + 3
                    i_cell_fr_setpoint = np.sum(rsp.spks[:, m.N_EXC:] > 0, axis=0)
                if i_e == epochs - 1:
                    sio.savemat(robustness_output_dir + '/' + f'title_{title}_dropout_{d_idx}_eidx_{zero_pad(i_e, 4)}', {
                        'first_spk_times': first_spk_times,
                        'w_r_e': rsp.ntwk.w_r['E'],
                    ...
                    spk_rates = np.sum(rsp.spks[:, :m.N_EXC] > 0, axis=0)
                    fr_update = np.zeros(spk_rates.shape)
                    fr_update[spk_rates > e_cell_fr_setpoint_upper] = a_u * (e_cell_fr_setpoint_upper - spk_rates[spk_rates > e_cell_fr_setpoint_upper])
                    fr_update[spk_rates < e_cell_fr_setpoint_lower] = a_l * (e_cell_fr_setpoint_lower - spk_rates[spk_rates < e_cell_fr_setpoint_lower])
                    # w_r['E'][m.N_EXC:, :m.N_EXC] += (0.5e-3 * (i_cell_fr_setpoint - np.sum(rsp.spks[:, m.N_EXC:] > 0, axis=0)) / i_cell_fr_setpoint * w_r['E'][m.N_EXC:, :m.N_EXC])
                    
                    stdp_update = 0
                    spk_time_hist = []
                    exc_spks = rsp.spks[:, :m.N_EXC]
                    for i in range(len(i_ext)):
                        stdp_update += ntwk.calc_stdp_update(i, exc_spks, S.DT, spk_time_hist)
                        ntwk.update_spk_hist(spk_time_hist, rsp.spks[i, :m.N_EXC], i)

                    print(fr_update)
                    print(b * stdp_update)
                    print(fr_update + b * stdp_update - c)
                    print(w_r['E'][:m.N_EXC, :m.N_EXC][w_r['E'][:m.N_EXC, :m.N_EXC] > 0])


                    w_r_e_update = (25e-6 * (fr_update + b * stdp_update - c) * np.where(w_r['E'][:m.N_EXC, :m.N_EXC] > 0, 1, 0))
                    w_r['E'][:m.N_EXC, :m.N_EXC] += w_r_e_update
                    w_r['E'][:m.N_EXC, :m.N_EXC][w_r['E'][:m.N_EXC, :m.N_EXC] < 0] = 1e-7
