...
def generate_local_con(m, ff_deg=[0, 1, 2], tempering=None):
    def unit_func():
        return np.random.rand(m.PROJECTION_NUM, m.PROJECTION_NUM)

    unit_funcs = [unit_func] * len(ff_deg)
    if tempering is None:
        tempering = [1.] * len(ff_deg)

    return m.RAND_WEIGHT_MAX * generate_ff_chain(m.N_EXC, m.PROJECTION_NUM, unit_funcs, ff_deg=ff_deg, tempering=tempering)
...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    if w_r_e is None:
        ...
        w_e_e_r += m.RAND_WEIGHT_MAX * generate_local_con(m, ff_deg=[0, 1], tempering=[0.3, 1.])
...
def quick_plot(m, run_title='', w_r_e=None, w_r_i=None, repeats=1, show_connectivity=True,
    n_show_only=None, add_noise=True, dropouts=[{'E': 0, 'I': 0}]):
    ...
    for idx_r, rsps in enumerate(all_rsps):
        for idx_do, rsp_for_dropout in enumerate(rsps):
            ...
            n_activations_to_compare = np.min([n_activations_to_compare, len(spk_timings['all'])])

            for i in range(1, n_activations_to_compare):
                early_timings = spk_timings['all'][0]
                late_timings = spk_timings['all'][i]

                timing_diffs = late_timings - early_timings

                gs = gridspec.GridSpec(1, 1)
                fig = plt.figure(figsize=(10, 6), tight_layout=True)
                axs = [fig.add_subplot(gs[0])]

                axs[0].scatter(np.arange(m.N_EXC), timing_diffs * 1000, s=5, color='red')

                axs[0].set_xlabel('Cell Index')
                axs[0].set_ylabel('Timing Difference (ms)')

                fig.savefig(f'{output_dir}/timing_diffs_{i}.png')
