...
def generate_prob_ff_chain(size, connection_func):
    cell_idxs = np.arange(size)
    mat = np.zeros((size, size))
    for idx in cell_idxs:
        mat[:, idx] = connection_func(size, idx)
    return mat

def normal_prob(u, s, x):
    return 1/(s * np.sqrt(2*pi)) * np.exp(-0.5*np.square((x-u)/s))

def generate_scaled_prof_ff_chain(m):
    scale = 20
    def connection_func(size, idx):
        outgoing_connections = np.where(np.random.rand(size) < (2 * normal_prob(idx/scale, 2, np.arange(size)/scale)), 1, 0)
        outgoing_connections[:(idx + 1)] = 0
        return outgoing_connections
    return m.W_INITIAL * generate_prob_ff_chain(m.N_EXC, connection_func)
...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    if w_r_e is None:
        w_e_e_r = generate_scaled_prof_ff_chain(m)
    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            active_connections_mask = None
            surviving_cell_indices = None
            dropout_activation_num = 10

            for i_e in range(epochs):

                if i_e == dropout_activation_num:
                    w_r['E'][:, :m.N_EXC], surviving_cell_indices = dropout_on_mat(w_r['E'][:, :m.N_EXC], dropout['E'], min_idx=m.PROJECTION_NUM)
                ...
                if surviving_cell_indices is not None:
                    ...
                    active_and_unperturbed_mask = np.isin(exc_raster[1, :], surviving_cell_indices.nonzero()[0])

                    exc_raster = exc_raster[:, active_and_unperturbed_mask]
                ...
                if i_e < dropout_activation_num:
                    ...
                    e_cell_fr_setpoints.append(np.sum(rsp.spks[:, :m.N_EXC] > 0, axis=0))

                else:
                    if i_e == dropout_activation_num:
                        active_connections_mask = w_r['E'][:m.N_EXC, :m.N_EXC] > 0
                        active_connections = np.where(active_connections_mask, 1, 0)
                        ...
                        e_cell_fr_setpoints = np.mean(np.stack(e_cell_fr_setpoints), axis=0)
                    else:
                        sio.savemat(robustness_output_dir + '/' + f'title_{title}_dropout_{d_idx}_eidx_{zero_pad(i_e, 4)}', {
                            'first_spk_times': first_spk_times,
                            'w_r_e': rsp.ntwk.w_r['E'],
                    ...
                    if i_e > dropout_activation_num:
                        ...
                        diffs[(diffs <= win_size_under) & (diffs >= -1 * win_size_over)] = 0
                        

                        w_r['E'][:m.N_EXC, :m.N_EXC] += (1e-6 * diffs * active_connections)
                        w_r['E'][:m.N_EXC, :m.N_EXC][active_connections_mask & (w_r['E'][:m.N_EXC, :m.N_EXC] <= 0)] = 1e-8

                        w_r['E'][:m.N_EXC, :m.N_EXC][w_r['E'][:m.N_EXC, :m.N_EXC] > 10 * m.W_INITIAL] = 10 * m.W_INITIAL
...
m2.W_E_I_R = 3e-5
m2.W_I_E_R = 1e-5 # 0.5e-5
m2.T_R_E = 1e-3
m2.W_MAX = 0.26 * 0.004 * 10
m2.W_INITIAL = 0.26 * 0.004 * 0.02
