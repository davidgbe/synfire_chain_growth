...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    if w_r_e is None:
        ...
        s_e_r = mat_1_if_under_val(0.05, (m.N_EXC, m.N_SILENT))
        e_s_r = mat_1_if_under_val(0.01, (m.N_SILENT, m.N_EXC))

        w_r_e = np.block([
            [ w_e_e_r, s_e_r * m.W_INITIAL / m.PROJECTION_NUM * 0.1, np.zeros((m.N_EXC, m.N_INH)) ],
            [ e_s_r * m.W_INITIAL / m.PROJECTION_NUM * 0.5, np.zeros((m.N_SILENT, m.N_SILENT + m.N_INH)) ],
            [ e_i_r * m.W_E_I_R,  np.zeros((m.N_INH, m.N_INH + m.N_SILENT)) ],
        ])

    if w_r_i is None:

        i_e_r = np.ones((m.N_EXC + m.N_SILENT, m.N_INH))

        w_r_i = np.block([
            [ np.zeros((m.N_EXC + m.N_SILENT, m.N_EXC + m.N_SILENT)), i_e_r * m.W_I_E_R ],
            [ np.zeros((m.N_INH, m.N_EXC + m.N_SILENT + m.N_INH)) ],
        ])
    ...
    w_r = {
        ...
        'I': w_r_i,
        'A': np.block([
            [ m.W_A * np.diag(np.ones((m.N_EXC + m.N_SILENT))), np.zeros((m.N_EXC + m.N_SILENT, m.N_INH)) ],
            [ np.zeros((m.N_INH, m.N_EXC + m.N_SILENT + m.N_INH)) ],
        ]),
    }
    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            for i_e in range(epochs):
                ...
                burst_t = np.arange(0, 6 * int(m.BURST_T / S.DT), int(m.BURST_T / S.DT))
                ...
                spk_bins, freqs = bin_occurrences(spks_for_e_cells.sum(axis=0), max_val=150, bin_size=1)
                if surviving_cell_indices is not None:
                ...
                first_spk_times, first_spk_order = process_single_activation(np.stack(spks_for_e_cells.nonzero()), m.N_EXC + m.N_SILENT)
                ...
                if active_cells_pre_dropout_mask is not None:
                    ...
                    exc_cells_initially_active_nonzero = exc_cells_initially_active.nonzero()
                    exc_cells_initially_active = np.stack([exc_cells_initially_active_nonzero[0], first_spk_order[exc_cells_initially_active_nonzero[1]]])

                    exc_cells_newly_active = copy(spks_for_e_cells)
                    exc_cells_newly_active[:, active_cells_pre_dropout_mask] = 0
                    exc_cells_newly_active_nonzero = exc_cells_newly_active.nonzero()
                    exc_cells_newly_active = np.stack([exc_cells_newly_active_nonzero[0], first_spk_order[exc_cells_newly_active_nonzero[1]]])

                    axs[0].scatter(exc_cells_initially_active[0, :] * S.DT * 1000, exc_cells_initially_active[1, :], s=1, c='black', zorder=0, alpha=0.3)
                ...
                axs[0].set_ylim(-1, m.N_EXC + m.N_SILENT + m.N_INH)
                axs[0].set_xlim(0, 0.1 * 1000)
                ...
                if i_e == 0:
                    ...
                    e_cell_fr_setpoints =  np.sum(rsp.spks[:, :(m.N_EXC + m.N_SILENT)] > 0, axis=0)
                else:
                    if i_e >= m.DROPOUT_ITER:
                    ...
                    t_steps_in_burst = int(5e-3/S.DT)
                    ...
                    for i_t in range(spks_for_e_cells.shape[0]):
                        ...
                        if t_points_for_stdp > 0:
                            ...
                            stdp_burst_pair_for_step = kron(curr_spks, sparse_spks).T.dot(m.KERNEL_PAIR[:t_points_for_stdp]).reshape(spks_for_e_cells.shape[1], spks_for_e_cells.shape[1])
                            stdp_burst_pair_for_step[stdp_burst_pair_for_step > 0] -= 0.4
                            stdp_burst_pair += stdp_burst_pair_for_step
                        else:
                            stdp_burst_pair += 0.
                    ...
                    total_potentiation = 1e-1 * (5e-2 * fr_update + 1e-3 * stdp_burst_pair + 1e-4 * fr_pop_update)

                    print('start')
                    print(5e-3 * np.sum(fr_update))
                    print(np.sum(stdp_burst_pair))
                    print(1e-4 * np.sum(fr_pop_update))
...
title = f'-0.4_stdp_ff_{clip(m2.W_INITIAL / (0.26 * 0.004))}_pf_{clip(m2.CON_PROB_FF, 2)}_pr_{clip(m2.CON_PROB_R, 2)}_eir_{clip(m2.W_E_I_R * 1e5)}_ier_{clip(m2.W_I_E_R * 1e5)}_dropout_sweep'
