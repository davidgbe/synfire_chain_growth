...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    if w_r_e is None:
        ...
        w_e_e_r += m.RAND_WEIGHT_MAX * generate_local_con(m, ff_deg=[0, 1])

        np.fill_diagonal(w_e_e_r, 0.)

        e_i_r = rand_per_row_mat(10, (m.N_INH, m.N_EXC))
    ...
    if w_r_i is None:
        ...
        con_per_e_cell = np.random.normal(loc=600, scale=0.1, size=m.N_EXC)
        con_per_e_cell[con_per_e_cell < 0] = 0
        con_per_e_cell[con_per_e_cell >= m.N_INH] = m.N_INH
        i_e_r = np.stack([rand_n_ones_in_vec_len_l(int(n_con), m.N_INH) for n_con in con_per_e_cell])
    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            rsp = ntwk.run(dt=S.DT, clamp=clamp, i_ext=i_ext,
                            ...
                            dropouts=[(m.DROPOUT_TIME, dropouts[d_idx])], m=m, repairs=[],
                            )
...
def quick_plot(m, run_title='', w_r_e=None, w_r_i=None, repeats=1, show_connectivity=True, n_show_only=None, add_noise=True, dropouts=[{'E': 0, 'I': 0}]):
    ...
    for idx_r, rsps in enumerate(all_rsps):
        for idx_do, rsp_for_dropout in enumerate(rsps):
            ...
            voltages = np.concatenate([rsp_for_dropout['t'].reshape(rsp_for_dropout['t'].shape[0], 1), rsp_for_dropout['vs']], axis=1)
            gs_e = np.concatenate([rsp_for_dropout['t'].reshape(rsp_for_dropout['t'].shape[0], 1), rsp_for_dropout['gs']['E']], axis=1)
            gs_i = np.concatenate([rsp_for_dropout['t'].reshape(rsp_for_dropout['t'].shape[0], 1), rsp_for_dropout['gs']['I']], axis=1)
            ...
            for title, trigger_times in plot_params.items():
                for t_idx, trigger_time in enumerate(trigger_times):
                    ...
                    voltages_for_win = voltages[(voltages[:, 0] >= t_window[0]) & (voltages[:, 0] < t_window[1]), :]
                    gs_e_for_win = gs_e[(gs_e[:, 0] >= t_window[0]) & (gs_e[:, 0] < t_window[1]), :]
                    gs_i_for_win = gs_i[(gs_i[:, 0] >= t_window[0]) & (gs_i[:, 0] < t_window[1]), :]
                    ...
                    start = m.PROJECTION_NUM * 50
                    exc_nrn_range = (start + 1, start + m.PROJECTION_NUM + 1)

                    for i in range(exc_nrn_range[0], exc_nrn_range[1]):
                        axs[1].plot(voltages_for_win[:, 0] * 1000, voltages_for_win[:, i], lw=0.5, color='purple')
                        axs[2].plot(gs_e_for_win[:, 0] * 1000, gs_e_for_win[:, i], lw=0.5, color='black')
                        axs[2].plot(gs_i_for_win[:, 0] * 1000, gs_i_for_win[:, i], lw=0.5, color='red')
                        
                    corrs_to_avg = []
                    for j in range(m.PROJECTION_NUM * 10, m.N_EXC):
                        corr = np.correlate(gs_e_for_win[:, j], gs_i_for_win[:, j], 'full')
                        if corr.max() > 2:
                            continue
                        corrs_to_avg.append(corr)
                    corrs_to_avg = np.stack(corrs_to_avg)
                    axs[3].fill_between((np.arange(corrs_to_avg.shape[1]) - corrs_to_avg.shape[1] / 2) * S.DT * 1000, corrs_to_avg.mean(axis=0) - corrs_to_avg.std(axis=0) / np.sqrt(corrs_to_avg.shape[0]), corrs_to_avg.mean(axis=0) + corrs_to_avg.std(axis=0) / np.sqrt(corrs_to_avg.shape[0]), lw=0.5, color='blue', alpha=0.2)
                    axs[3].plot((np.arange(corrs_to_avg.shape[1]) - corrs_to_avg.shape[1] / 2) * S.DT * 1000, corrs_to_avg.mean(axis=0), lw=0.8, color='blue')

                    axs[1].set_xlabel('Time (ms)')
                    axs[1].set_ylabel('Voltage (V)')

                    axs[2].set_xlabel('Time (ms)')
                    axs[2].set_ylabel('Relative Conductance')
...
m2.W_INITIAL = 0.26 * 0.004 * 0.75
m2.W_U_E = 0.26 * 0.004 * .25
m2.M = 10
