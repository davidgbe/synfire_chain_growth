    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            for i_e in range(epochs):
                ...
                if i_e == 1:
                    w_r['E'][:, (20 * m.PROJECTION_NUM):m.N_EXC] = dropout_on_mat(w_r['E'][:, (20 * m.PROJECTION_NUM):m.N_EXC], dropout['E'])
                ...
                if i_e == 0:
                    ...
                    e_cell_fr_setpoints =  np.sum(rsp.spks[:, (20 * m.PROJECTION_NUM):m.N_EXC] > 0, axis=0)
                if i_e == epochs - 1:
                    sio.savemat(robustness_output_dir + '/' + f'title_{title}_dropout_{d_idx}_eidx_{zero_pad(i_e, 4)}', {
                        'first_spk_times': first_spk_times,
                        'w_r_e': rsp.ntwk.w_r['E'],
                    ...
                    w_r['E'][(20 * m.PROJECTION_NUM):m.N_EXC, (20 * m.PROJECTION_NUM):m.N_EXC] += (1e-2 * (e_cell_fr_setpoints - np.sum(rsp.spks[:, (20 * m.PROJECTION_NUM):m.N_EXC] > 0, axis=0)) * w_r['E'][(20 * m.PROJECTION_NUM):m.N_EXC, (20 * m.PROJECTION_NUM):m.N_EXC])
