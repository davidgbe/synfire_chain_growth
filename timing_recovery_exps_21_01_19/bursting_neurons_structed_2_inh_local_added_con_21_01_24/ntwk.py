...
class LIFNtwkG(object):
    ...
    def __init__(self, c_m, g_l, e_l, v_th, v_r, t_r, e_s, t_s, w_r, w_u, plasticity_indices, connectivity, W_max, m, eta,
        epsilon, dt, gamma, alpha, fr_set_points, fr_baseline, stdp_scale, bursting_cell_indices,
        ...
        self.bursting_cell_indices = bursting_cell_indices
    ...
    def update_w(self, t_ctr, spks, dt, spk_time_hist):
        ...
        fr_diffs = spk_counts - self.fr_set_points
        fr_homeo = self.fr_baseline * np.ones(fr_diffs.shape)
        fr_homeo[fr_diffs >= 0] -= self.alpha * np.square(fr_diffs[fr_diffs >= 0])
        ...
        weight_update = self.eta * (delta_stdp + fr_homeo_update)
    ...
    def run(self, dt, clamp, i_ext, output_dir_name, m, dropouts=[], repairs=[], spks_u=None):
        """
        Run simulation.
        ...
        # loop over timesteps
        for t_ctr in range(len(i_ext) - 4 * m.BURST_C):
            ...
            for t in repairs:
                if int(t / dt) == t_ctr:
                    current_w_r_e = copy(self.w_r['E'][:m.N_EXC, :m.N_EXC])
                    for i in range(4500):
                        outgoing_strengths = self.w_r['E'][:m.N_EXC, :m.N_EXC].sum(axis=1).reshape(m.N_EXC, 1)
                        self.w_r['E'][:m.N_EXC, :m.N_EXC] += 0.007 * current_w_r_e
            ...
            if t_ctr in clamp.spk:  # check for clamped spikes
                ...
                bursting_spks_for_t_ctr = spks_for_t_ctr * (self.bursting_cell_indices == 1)
                nonbursting_spks_for_t_ctr = spks_for_t_ctr * (self.bursting_cell_indices == 0)

                for burst_idx in range(4):
                    spks[t_ctr + burst_idx * m.BURST_C, bursting_spks_for_t_ctr] = 1

                spks[t_ctr, nonbursting_spks_for_t_ctr] = 1
