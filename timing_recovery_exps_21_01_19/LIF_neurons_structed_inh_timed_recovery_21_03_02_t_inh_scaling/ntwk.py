...
class LIFNtwkG(object):
    ...
    def update_w(self, t_ctr, spks, dt, spk_time_hist):
        ...
        # spk_counts = spks[-int(self.tau_fr/dt):, :].sum(axis=0)
        fr_diffs = (self.fr_set_points - curr_spks)
        fr_homeo = self.alpha * fr_diffs
        fr_homeo = fr_homeo.reshape(fr_homeo.shape[0], 1)
        fr_homeo_update = fr_homeo * w_r_e_plastic / self.w_max

        input_setpoint_update = self.beta * (0.5 * self.W_max - np.sum(w_r_e_plastic, axis=1)) #/ (0.5 * self.W_max) 
        input_setpoint_update = input_setpoint_update.reshape(input_setpoint_update.shape[0], 1) * w_r_e_plastic / self.w_max
        ...
        # update weights with firing rate and STDP rules
        weight_update = self.eta * (delta_stdp + fr_homeo_update + input_setpoint_update)
        np.fill_diagonal(weight_update, 0.)
        self.w_r['E'][self.w_r_e_plastic_mask] += weight_update.flatten()

        # clip negative synapses
    ...
    def run(self, dt, clamp, i_ext, output_dir_name, dropouts, m, repairs=[], spks_u=None):
        ...
        avg_initial_input_per_cell_e_i = np.mean(self.w_r['E'][m.N_EXC:, :m.N_EXC].sum(axis=1))
        ...
        for t_ctr in range(len(i_ext) - 4 * t_r_int[0]):
            ...
            if self.output and (t_ctr == 0):
                sio.savemat(output_dir + '/' + f'{zero_pad(0, 6)}', {
                    ...
                    'avg_input_per_cell_e_i':  avg_initial_input_per_cell_e_i,
                })
            ...
            for t, dropout in dropouts:
                if int(t / dt) == t_ctr:
                    ...
                    avg_input_per_cell_e_i = np.mean(self.w_r['E'][m.N_EXC:, :m.N_EXC].sum(axis=1))

                    if self.output:
                        sio.savemat(output_dir + '/' + f'{zero_pad(1, 6)}', {
                            'avg_input_per_cell': avg_input_per_cell,
                            'avg_input_per_cell_e_i': avg_input_per_cell_e_i,
                        })
            
            for i_r, (t, repair_setpoint, repair_type) in enumerate(repairs):
                if int(t / dt) == t_ctr:
                    avg_input_per_cell = np.mean(self.w_r['E'][:m.N_EXC, :m.N_EXC].sum(axis=1))
                    avg_input_per_cell_e_i = np.mean(self.w_r['E'][m.N_EXC:, :m.N_EXC].sum(axis=1))

                    print(repair_setpoint, repair_type)
                    ...
                    if repair_type == 'ee':
                        ...
                        target = repair_setpoint * avg_initial_input_per_cell_e_i
                        for i in range(1000):
                            avg_input_per_cell_e_i = np.mean(self.w_r['E'][m.N_EXC:, :m.N_EXC].sum(axis=1))
                            self.w_r['E'][m.N_EXC:, :m.N_EXC] += 100. * self.w_r['E'][m.N_EXC:, :m.N_EXC] * (target - avg_input_per_cell_e_i)
