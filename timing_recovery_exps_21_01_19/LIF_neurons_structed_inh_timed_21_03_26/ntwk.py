    ...
    def run(self, dt, clamp, i_ext, output_dir_name, dropouts, m, repairs=[], spks_u=None):
        ...
        for t_ctr in range(len(i_ext) - 4 * t_r_int[0]):
            ...
            if self.output and t_ctr == 0:
                sio.savemat(output_dir + '/' + f'{zero_pad(0, 3)}', {
                    'w_r_e': self.w_r['E'],
                    'w_r_i': self.w_r['I'],
                })
            ...
            for t, dropout in dropouts:
                if int(t / dt) == t_ctr:
                    ...
                    if self.output:
                        sio.savemat(output_dir + '/' + f'{zero_pad(1, 3)}', {
                            'w_r_e': self.w_r['E'],
                            'w_r_i': self.w_r['I'],
                        })

            for i_t, t in enumerate(repairs):
                if int(t / dt) == t_ctr:
                    targets = np.ones((m.N_EXC)) * m.W_INITIAL
                    targets[m.PROJECTION_NUM * 5:] = 1.5 *  m.W_INITIAL
                    for i in range(1000): 
                        outgoing_strengths = self.w_r['E'][:m.N_EXC, :m.N_EXC].sum(axis=1).reshape(m.N_EXC, 1)
                        self.w_r['E'][:m.N_EXC, :m.N_EXC] += 100. * self.w_r['E'][:m.N_EXC, :m.N_EXC] * (targets - outgoing_strengths)
                        over_w_max = self.w_r['E'][:m.N_EXC, :m.N_EXC] > self.w_max
                        self.w_r['E'][:m.N_EXC, :m.N_EXC][over_w_max] = self.w_max


                    if self.output:
                        sio.savemat(output_dir + '/' + f'{zero_pad(i_t + 2, 3)}', {
                            'w_r_e': self.w_r['E'],
                            'w_r_i': self.w_r['I'],
                        })
