...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    if w_r_e is None:
        ...
        e_i_r = generate_ff_chain(m.N_EXC, m.PROJECTION_NUM, [rand_unit_func] * 1, ff_deg=[2], tempering=[1.])
    ...
    if w_r_i is None:
        ...
        i_e_r = np.random.rand(m.N_EXC, m.N_INH) - generate_ff_chain(m.N_INH, m.PROJECTION_NUM, i_e_unit_funcs, ff_deg=ff_deg, tempering=[.7, 1., 1., .5])
        i_e_r[i_e_r < 0] = 0
