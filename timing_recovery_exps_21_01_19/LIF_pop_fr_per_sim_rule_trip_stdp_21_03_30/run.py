...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            e_pop_fr_setpoint_lower = None
            e_pop_fr_setpoint_upper = None
            i_cell_fr_setpoint = None

            for i_e in range(epochs):

                if i_e == 1:
                    w_r['E'][:, m.PROJECTION_NUM:m.N_EXC], e_d_indices = dropout_on_mat(w_r['E'][:, m.PROJECTION_NUM:m.N_EXC], dropout['E'])
                    w_r['I'][:, m.N_EXC:], i_d_indices = dropout_on_mat(w_r['I'][:, m.N_EXC:], dropout['I'])
                ...
                if i_e == 0:
                    ...
                    e_pop_fr_setpoint_lower = np.sum(rsp.spks[:, :m.N_EXC] > 0)
                    e_pop_fr_setpoint_upper = e_pop_fr_setpoint_lower + m.N_EXC * 3
                ...
                if i_e > 1:
                    ...
                    fr_update = 0
                    tot_spks = np.sum(rsp.spks > 0)
                    print(tot_spks, e_pop_fr_setpoint_lower, e_pop_fr_setpoint_upper)
                    if tot_spks > e_pop_fr_setpoint_upper:
                        fr_update = a_u * (e_pop_fr_setpoint_upper - tot_spks)
                    elif tot_spks < e_pop_fr_setpoint_lower:
                        fr_update = a_l * (e_pop_fr_setpoint_lower - tot_spks)
                        print((e_pop_fr_setpoint_lower - tot_spks))
                    ...
                    w_r_e_update = (10e-6 * (fr_update + b * stdp_update - c) * np.where(w_r['E'][:m.N_EXC, :m.N_EXC] > 0, 1, 0))
