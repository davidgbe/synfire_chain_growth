...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    if w_r_e is None:
        ...
        w_r_e = np.block([
            ...
            [ e_s_r * m.W_INITIAL / m.PROJECTION_NUM, np.zeros((m.N_SILENT, m.N_SILENT + m.N_INH)) ],
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            for i_e in range(epochs):
                ...
                if i_e == 0:
                    ...
                    total_potentiation = 3e-2 * fr_update + 1e-3 * stdp_burst_pair + 1e-4 * fr_pop_update
...
def process_single_activation(exc_raster, m):
    ...
    first_spk_times = np.nan * np.ones(m.N_EXC + m.N_SILENT)
