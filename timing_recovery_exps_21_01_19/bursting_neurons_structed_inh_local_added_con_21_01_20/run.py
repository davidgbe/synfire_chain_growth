...
def generate_chain_weight_mat(m):
    ...
    for idx, layer_idx in enumerate(chain_order):
        ...
        w_syn[layer_start : layer_start + m.PROJECTION_NUM, layer_start : layer_start + m.PROJECTION_NUM] = rand_per_row_mat(m.CON_PER_ROW - m.CON_PER_ROW_FF, (m.PROJECTION_NUM, m.PROJECTION_NUM))
        if idx + 1 < len(chain_order):
            next_layer_idx = chain_order[idx + 1]
            next_layer_start = m.PROJECTION_NUM * next_layer_idx

            w_syn[next_layer_start : next_layer_start + m.PROJECTION_NUM, layer_start : layer_start + m.PROJECTION_NUM] = rand_per_row_mat(m.CON_PER_ROW_FF, (m.PROJECTION_NUM, m.PROJECTION_NUM))
...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    if w_r_e is None:
        ...
        w_e_e_r += m.RAND_WEIGHT_MAX * np.where(rand_connectivity_mask, np.random.rand(m.N_EXC, m.N_EXC), 0)

        np.fill_diagonal(w_e_e_r, 0.)

        e_i_r = block_diag(*[np.ones((m.PROJECTION_NUM, m.PROJECTION_NUM)) for i in range(int(m.N_EXC / m.PROJECTION_NUM))])
    ...
    def gen_inh_output(i, m):
        output = np.zeros((m.N_EXC))
        tau = 10.
        output[:i] = np.exp(-(i - np.arange(i)) / tau)
        output[i:] = 1 - np.exp((i - np.arange(i, m.N_EXC)) / tau)
        return output

    if w_r_i is None:
        i_e_r = np.stack([gen_inh_output(i, m) for i in range(m.N_INH)]).T
    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            rsp = ntwk.run(dt=S.DT, clamp=clamp, i_ext=i_ext,
                            ...
                            dropouts=[(m.DROPOUT_TIME, dropouts[d_idx])], repairs=[m.REPAIR_TIME], m=m,
                            )
