...
def process_single_activation(raster, size, default=-1):
    # extract first spikes
    first_spk_times = np.nan * np.ones(size)
    x = np.nan * np.ones(size)
    i = 0

    for t_idx in range(raster.shape[1]):
        nrn_idx = int(raster[1, t_idx])
        if np.isnan(first_spk_times[nrn_idx]):
            x[nrn_idx] = i
            first_spk_times[nrn_idx] = raster[0, t_idx]
            i += 1
    return first_spk_times, x
