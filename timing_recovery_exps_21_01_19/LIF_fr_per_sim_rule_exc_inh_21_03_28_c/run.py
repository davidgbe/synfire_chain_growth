        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            for i_e in range(epochs):
                ...
                if i_e == 0:
                    ...
                    e_cell_fr_setpoint =  np.mean(np.sum(rsp.spks[:, :m.N_EXC] > 0, axis=0))
                    i_cell_fr_setpoint =  np.mean(np.sum(rsp.spks[:, m.N_EXC:] > 0, axis=0))
                if i_e == epochs - 1:
                    sio.savemat(robustness_output_dir + '/' + f'title_{title}_dropout_{d_idx}_eidx_{zero_pad(i_e, 4)}', {
                        'first_spk_times': first_spk_times,
                        'w_r_e': rsp.ntwk.w_r['E'],
                    ...
                    w_r['E'][:m.N_EXC, :m.N_EXC] += (3e-3 * (e_cell_fr_setpoint - np.sum(rsp.spks[:, :m.N_EXC] > 0, axis=0)) / e_cell_fr_setpoint * w_r['E'][:m.N_EXC, :m.N_EXC])
...
m2.BURST_T = 2e-3
m2.CON_PROB_FF = args.p_ff[0]
m2.CON_PROB_R = args.p_r[0]
