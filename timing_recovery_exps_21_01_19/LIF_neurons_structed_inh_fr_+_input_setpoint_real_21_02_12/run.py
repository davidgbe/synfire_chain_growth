...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    if w_r_e is None:
        ...
        e_i_r = generate_ff_chain(m.N_INH, m.PROJECTION_NUM, [solid_unit_func], ff_deg=[1], tempering=[1.])
    ...
    if w_r_i is None:
        ...
        ff_deg = np.arange(3, 12)
        i_e_unit_funcs = [solid_unit_func] * len(ff_deg)
        inh_filters = generate_ff_chain(m.N_INH, m.PROJECTION_NUM, i_e_unit_funcs, ff_deg=ff_deg, tempering=np.ones(len(ff_deg)))
