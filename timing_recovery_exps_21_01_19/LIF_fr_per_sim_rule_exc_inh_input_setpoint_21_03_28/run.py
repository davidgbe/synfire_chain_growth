...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    add_noise=True, dropouts=[{'E': 0, 'I': 0}], w_r_e=None, w_r_i=None, epochs=300):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            for i_e in range(epochs):
                ...
                if i_e == 0:
                    ...
                    i_cell_input_setpoint =  np.mean(np.sum(w_r['E'][m.N_EXC:, :m.N_EXC], axis=1))
                else:
                    sio.savemat(robustness_output_dir + '/' + f'title_{title}_dropout_{d_idx}_eidx_{zero_pad(i_e, 4)}', {
                        'first_spk_times': first_spk_times,
                        'w_r_e': rsp.ntwk.w_r['E'],
                    ...
                    w_r['E'][m.N_EXC:, :m.N_EXC] += ((2e-3 * (i_cell_input_setpoint - np.sum(w_r['E'][m.N_EXC:, :m.N_EXC], axis=1)) / i_cell_input_setpoint).reshape(m.N_INH, 1) * w_r['E'][m.N_EXC:, :m.N_EXC])
