...
def generate_scaled_prof_ff_chain(m):
    ...
    def connection_func(size, idx):
        outgoing_connections = np.where(np.random.rand(size) < (2 * normal_prob(idx/scale, 1, np.arange(size)/scale)), 1, 0)
            ...
            for i_e in range(epochs):
                ...
                if i_e < dropout_activation_num:
                    ...
                    if i_e > dropout_activation_num:
                        ...
                        w_r['E'][:m.N_EXC, :m.N_EXC] += (0.5e-6 * diffs * active_connections)
