        ...
        for idx_do, rsp_for_dropout in enumerate(rsps):
            ...
            for title, trigger_times in plot_params.items():
                for t_idx, trigger_time in enumerate(trigger_times):
                    ...
                    sio.savemat(robustness_output_dir + '/' + f'title_{title}_dropout_{idx_do}_tidx_{zero_pad(t_idx, 2)}', {
                        'prop': popt[0],
                        'success': success,
                    ...
                    fig.savefig(f'{output_dir}/{title}_{idx_r}_{idx_do}_{zero_pad(t_idx, 2)}.png')
...
repair_setpoints = 0.5 * np.arange(n_repairs) + 1.0
m2.REPAIRS = [(rep_time, repair_setpoints, 'ee') for rep_time, repair_setpoints in zip(repair_times, repair_setpoints)]
m2.REPAIRS.append((0.300 * n_repairs + 0.595, 1.0, 'ei'))
