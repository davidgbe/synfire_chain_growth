...
def generate_exc_ff_chain(m):
    ...
    return m.W_INITIAL / m.CON_PER_ROW * generate_ff_chain(m.N_EXC, m.PROJECTION_NUM, unit_funcs)
...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    if w_r_i is None:
        ...
        i_e_r = np.zeros((m.N_EXC, m.N_INH))

        ff_deg = [0, 1, 2, 3, 4]
        i_e_unit_funcs = [solid_unit_func] * len(ff_deg)
        inh_filters = generate_ff_chain(m.N_INH, m.PROJECTION_NUM, i_e_unit_funcs, ff_deg=ff_deg, tempering=[.7, 1., 1., 1., .5])

        for r_idx in range(i_e_cons.shape[0]):
            i_e_r[r_idx, :] = inh_filters[i_e_cons[r_idx], :].sum(axis=0)

        i_e_r = np.ones((m.N_EXC, m.N_INH)) - i_e_r
        i_e_r[i_e_r < 0] = 0
...
m2.W_I_E_R = 0.8e-5
m2.T_R_E = 1e-3
m2.W_MAX = 0.26 * 0.004 * 1.2
m2.W_INITIAL = 0.26 * 0.004 * .5
...
fr_set_points = [4. * m2.DRIVING_HZ * S.DT]
