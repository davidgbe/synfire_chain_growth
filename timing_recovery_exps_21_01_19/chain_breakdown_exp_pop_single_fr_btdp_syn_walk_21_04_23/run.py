...
def generate_exc_ff_chain(m):
    ...
    def ff_unit_func():
        w = m.W_INITIAL / m.PROJECTION_NUM * 0.4 / m.CON_PROB_FF
...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    w_u = {
        ...
        'E': np.block([
            [ w_u_e, np.zeros([n_cells_driven, m.N_EXC + m.N_SILENT + m.N_INH]) ],
            [ np.zeros([m.N_EXC + m.N_SILENT + m.N_INH - n_cells_driven, m.N_EXC + m.N_SILENT + m.N_INH + m.N_DRIVING_CELLS]) ],
        ]),
    ...
    if w_r_e is None:
        ...
        w_e_e_r = np.where(np.random.rand(m.N_EXC + m.N_SILENT, 1) < 0.5, (np.random.rand(m.N_EXC + m.N_SILENT, 1) * .15 + 0.1), 1) * w_e_e_r
    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            for i_e in range(epochs):
                ...
                fig = plt.figure(figsize=(9 * scale, 12 * scale), tight_layout=True)
                axs = [fig.add_subplot(gs[:2]), fig.add_subplot(gs[2]), fig.add_subplot(gs[3])]
                ...
                e_cell_spks_counts_for_activation = spks_for_e_cells.sum(axis=0)
                ...
                e_cell_participation = np.where(e_cell_spks_counts_for_activation > 0, 1, 0)
                e_cell_participation_hist.append(e_cell_participation)

                participation_percentages = np.stack(e_cell_participation_hist).sum(axis=0) / len(e_cell_participation_hist)

                axs[2].bar(*(bin_occurrences(participation_percentages, max_val=1, bin_size=0.05)), width=0.05)
                ...
                if active_cells_pre_dropout_mask is not None:
                    ...
                    axs[0].scatter(exc_cells_newly_active[0, :] * S.DT * 1000, exc_cells_newly_active[1, :], s=3, c='green', zorder=1, alpha=1)
                else:
                    exc_raster = raster[:, raster[1, :] < (m.N_EXC + m.N_SILENT)]

                    axs[0].scatter(exc_raster[0, :] * 1000, exc_raster[1, :], s=1, c='black', zorder=0, alpha=1)

                axs[0].scatter(inh_raster[0, :] * 1000, inh_raster[1, :] - m.N_SILENT, s=1, c='red', zorder=0, alpha=1)
                ...
                if i_e == 0:
                    ...
                    total_potentiation = 1e-2 * fr_update + 1e-3 * stdp_burst_pair + 1e-4 * fr_pop_update + 1e-1 * np.random.normal(loc=0, scale=1, size=(m.N_EXC + m.N_SILENT, m.N_EXC + m.N_SILENT))
