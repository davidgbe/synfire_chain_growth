...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    add_noise=True, dropouts=[{'E': 0, 'I': 0}], w_r_e=None, w_r_i=None, epochs=100):
    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            t_window = (prop_start, prop_start + 250e-3)
            ...
            # i_e_r[np.random.rand(*(i_e_r.shape)) < 0.1] = 0
            ...
            for i_e in range(epochs):
                ...
                rsp = ntwk.run(dt=S.DT, clamp=clamp, i_ext=inputs[sim_num]['i_ext'],
                                output_dir_name=f'{output_dir_name}_{d_idx}_{i_e}', spks_u=inputs[sim_num]['spks_u'],
                ...
                if i_e == 0:
                    ...
                    fr_update[spk_rates > e_cell_fr_setpoint_upper] = (e_cell_fr_setpoint_upper - spk_rates[spk_rates > e_cell_fr_setpoint_upper])
                    fr_update[spk_rates < e_cell_fr_setpoint_lower] = (e_cell_fr_setpoint_lower - spk_rates[spk_rates < e_cell_fr_setpoint_lower])

                    w_r['E'][:m.N_EXC, :m.N_EXC] += ((5e-2 * fr_update) * w_r['E'][:m.N_EXC, :m.N_EXC])
