...
def ff_exp_decay_kernel(dist, tau, weight):
    if len(dist.shape) == 0:
        dist = np.array(dist).reshape(1)
    return np.where((dist > 0) & (np.random.rand(dist.shape[0]) <= np.exp(-dist/tau)), 1, 0) * weight

def generate_prob_ff_chain(size, kernel):
    cell_locs = np.arange(size)

    mat = np.zeros((size, size))
    for i in range(size):
        for j in range(size):
            dist = cell_locs[i] - cell_locs[j]
            mat[i, j] = kernel(dist)
    return mat

def generate_prob_exp_ff_chain(size, tau, weight):
    def kernel(dist):
        return ff_exp_decay_kernel(dist, tau, weight)

    return generate_prob_ff_chain(size, kernel)
...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    if w_r_e is None:
        w_e_e_r = generate_prob_exp_ff_chain(m.N_EXC, m.TAU_CON, m.W_INITIAL / m.CON_PER_ROW)
        ...
        for idx_do, rsp_for_dropout in enumerate(rsps):
            ...
            for title, trigger_times in plot_params.items():
                for t_idx, trigger_time in enumerate(trigger_times):
                    ...
                    t_window = (trigger_time * S.DT, trigger_time * S.DT + 110e-3)
