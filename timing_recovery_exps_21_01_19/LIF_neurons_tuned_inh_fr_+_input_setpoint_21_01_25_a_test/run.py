...
def run_test(m, output_dir_name, show_connectivity=True, repeats=1, n_show_only=None,
    ...
    if w_r_i is None:
        i_e_r = np.stack([rand_n_ones_in_vec_len_l(150, m.N_INH) for i in range(m.N_EXC)])
        ...
        for idx_do, rsp_for_dropout in enumerate(rsps):
            ...
            for title, trigger_times in plot_params.items():
                for t_idx, trigger_time in enumerate(trigger_times):
                    ...
                    t_window = (trigger_time * S.DT, trigger_time * S.DT + 50e-3)
...
w_r_e, w_r_i = load_weight_matrices('./data/stability_test_2021-01-25--12:26--00:5260_0_0', -1)
