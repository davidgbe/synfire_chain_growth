...
M = Generic(
    ...
    SETPOINT_MEASUREMENT_PERIOD=(360, 390),
)
...
def run_test(m, output_dir_name, n_show_only=None, add_noise=True, dropout={'E': 0, 'I': 0},
    ...
    for i_e in range(epochs):
        ...
        if i_e > 0:
            ...
            filtered_spks_for_i_cells = spks_for_i_cells
            filtered_spks_received_for_i_cells = spks_received_for_i_cells
            ...
            for i_t in range(spks_for_e_cells.shape[0]):
                ...
                for curr_spk_e in curr_spks_e.nonzero()[0]:
                    ...
                    sparse_spks_received_i_plus = csc_matrix(filtered_spks_received_for_i_cells[stdp_start_ee:i_t, curr_spk_e, :])
                    sparse_spks_received_i_minus = csc_matrix(filtered_spks_received_for_i_cells[i_t:stdp_end_ee, curr_spk_e, :])
                    ...
                    stdp_burst_pair_e_i_minus[:, curr_spk_e] += sparse_spks_received_i_minus.T.dot(trimmed_kernel_ee_minus)
                    ...
                    for k_t in trip_spk_hist[curr_spk_e]:
                        ...
                        stdp_burst_pair_e_i_plus[:, curr_spk_e] += stdp_burst_pair_e_i_outer_plus[:, -(i_t - k_t):].sum(axis=1) * m.KERNEL_TRIP_EE[i_t - k_t]
            ...
            if m.STDP_TYPE == 'mult':
                ...
                w_r_copy['E'][:m.N_INH, :(m.N_EXC + m.N_UVA)] += (m.ETA * m.BETA_2 * m.A_PAIR_MINUS * stdp_burst_pair_e_i_minus * w_r_copy['E'][:(m.N_INH), :(m.N_EXC + m.N_UVA)])
                w_r_copy['E'][:m.N_INH, :(m.N_EXC + m.N_UVA)] += (m.ETA * m.BETA_2 * m.A_TRIP_PLUS * stdp_burst_pair_e_j_plus * (m.W_E_I_R_MAX * ei_connectivity - w_r_copy['E'][:m.N_INH, :(m.N_EXC + m.N_UVA)]))
