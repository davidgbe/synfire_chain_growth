...
def ff_unit_func(m, p):
    ...
    return gaussian_if_under_val(p, (m.PROJECTION_NUM, m.PROJECTION_NUM), w, 0)
...
def run_test(m, output_dir_name, n_show_only=None, add_noise=True, dropout={'E': 0, 'I': 0},
    ...
    if w_r_e is None:
        ...
        for l_idx in range(con_reach):
            unit_funcs.append(partial(ff_unit_func, m=m, p=np.max(0.7 - l_idx * 0.15, 0)))
