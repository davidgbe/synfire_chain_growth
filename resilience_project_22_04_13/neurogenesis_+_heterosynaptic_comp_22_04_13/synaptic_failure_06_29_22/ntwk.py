...
from aux import Generic, c_tile, r_tile, dropout_on_mat, mat_1_if_under_val
...
class LIFNtwkG(object):
    ...
    def __init__(self, c_m, g_l, e_l, v_th, v_r, t_r, e_s, t_s, w_r, w_u, pairwise_spk_delays, delay_map, syn_fail_prob=0, sparse=True):
        # ntwk size
        ...
        self.v_r = v_r
        self.t_r = t_r
        self.syn_fail_prob = syn_fail_prob
    ...
    def run(self, dt, clamp, i_ext, spks_u=None):
        ...
        for t_ctr in range(len(i_ext)):
            ...
            if t_ctr in clamp.spk:  # check for clamped spikes
                ...
                successes = mat_1_if_under_val(1. - self.syn_fail_prob, vs[t_ctr, :].shape).astype(bool)
                spks_for_t_ctr = vs[t_ctr, :] >= v_th
                successful_spks_for_t_ctr = spks_for_t_ctr & successes
                spks[t_ctr, successful_spks_for_t_ctr] = 1
                nonzero_spks = successful_spks_for_t_ctr.nonzero()[0]
                if len(nonzero_spks) > 0:
                    for k in nonzero_spks:
                        spks_received[self.delay_map[k][0] + t_ctr - 1, self.delay_map[k][1], k] = 1
                # reset v and update refrac periods for nrns that spiked
                vs[t_ctr, spks_for_t_ctr] = v_r[spks_for_t_ctr]
                rp_ctr[spks_for_t_ctr] = t_r_int[spks_for_t_ctr] + 1
