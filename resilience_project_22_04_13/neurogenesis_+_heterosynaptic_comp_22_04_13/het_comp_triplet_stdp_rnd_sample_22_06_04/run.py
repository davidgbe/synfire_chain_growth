...
M = Generic(
    ...
    SETPOINT_MEASUREMENT_PERIOD=(0, 30),
)

print(M.HETERO_COMP_MECH)

S = Generic(RNG_SEED=args.rng_seed[0], DT=0.05e-3, T=280e-3, EPOCHS=5000)
...
def gen_continuous_network(size, m):
    ...
    active_inactive_pairings = np.outer(active_cell_mask, active_cell_mask)
    cont_idx_dists = cont_idx.reshape(cont_idx.shape[0], 1) - cont_idx.reshape(1, cont_idx.shape[0])

    def exp_if_pos(dist, tau):
        return np.where(dist >= 0, np.exp(-dist/tau), 0)

    mat = np.where(active_inactive_pairings, w * exp_if_pos(cont_idx_dists, 5), gaussian_if_under_val(0.05, (size, size), 0.05 * w, 0.005 * w))
...
def run_test(m, output_dir_name, n_show_only=None, add_noise=True, dropout={'E': 0, 'I': 0},
    ...
    if w_r_e is None:
        ...
        w_e_e_r_chain = gen_continuous_network(m.N_EXC_OLD, m)
    ...
    n_neighbor_samplings = gaussian_if_under_val(1, (m.N_EXC,), 5., 1.).astype(int)
    neighbor_samplings = np.concatenate([rand_mat_per_row(n_n_s, (1, m.N_EXC)) for n_n_s in n_neighbor_samplings], axis=0).astype(bool)


    pairwise_spk_delays = np.block([
        [(3.5e-3 / S.DT * spatial_dists).astype(int), np.ones((m.N_EXC, m.N_UVA)), int(0.5e-3 / S.DT) * np.ones((m.N_EXC, m.N_INH))],
    ...
    for i_e in range(epochs):
        ...
        if not args.cond[0].startswith('no_repl_no_syn'):
            if i_e in m.RANDOM_SYN_ADD_ITERS_EE:
                new_synapses = exp_if_under_val(2 * growth_prob, (m.N_EXC, m.N_EXC), 0.4 * m.W_E_E_R / M.PROJECTION_NUM)
        ...
        if i_e > m.SETPOINT_MEASUREMENT_PERIOD[1]:
            ...
            if m.STDP_TYPE == 'mult':
                w_r_copy['E'][:m.N_EXC, :(m.N_EXC + m.N_UVA)] += (m.ETA * m.BETA_1 * m.A_PAIR_MINUS * stdp_burst_pair_e_e_minus * w_r_copy['E'][:(m.N_EXC), :(m.N_EXC + m.N_UVA)])
                w_r_copy['E'][:m.N_EXC, :(m.N_EXC + m.N_UVA)] += (m.ETA * m.BETA_1 * m.A_TRIP_PLUS * stdp_burst_pair_e_e_plus * w_r_copy['E'][:(m.N_EXC), :(m.N_EXC + m.N_UVA)])
            else:
                w_r_copy['E'][:m.N_EXC, :(m.N_EXC + m.N_UVA)] += (m.ETA * m.BETA_2 * m.A_PAIR_MINUS * stdp_burst_pair_e_e_minus * w_r_copy['E'][:(m.N_EXC), :(m.N_EXC + m.N_UVA)])
                w_r_copy['E'][:m.N_EXC, :(m.N_EXC + m.N_UVA)] += (m.ETA * m.BETA_2 * m.A_TRIP_PLUS * stdp_burst_pair_e_e_plus * (m.W_E_E_R_MAX * ee_connectivity - w_r_copy['E'][:(m.N_EXC), :(m.N_EXC + m.N_UVA)]))
            ...
            if m.HETERO_COMP_MECH.startswith('firing_rate'):
                ...
                # sigmoid_transform_e_diffs = e_diffs
                # sigmoid_transform_e_diffs[sigmoid_transform_e_diffs > 0] = sigmoid_tranform(sigmoid_transform_e_diffs[sigmoid_transform_e_diffs > 0])
            ...
            if m.HETERO_COMP_MECH.startswith('sampled_regulation'):
                if i_e >= m.DROPOUT_ITER:
                ...
                secreted_diffs[secreted_diffs < 0] = np.power(secreted_diffs[secreted_diffs < 0], 3)
                w_update = secreted_diffs.reshape(secreted_diffs.shape[0], 1) * np.ones((m.N_EXC, m.N_EXC + m.N_UVA)).astype(float)
