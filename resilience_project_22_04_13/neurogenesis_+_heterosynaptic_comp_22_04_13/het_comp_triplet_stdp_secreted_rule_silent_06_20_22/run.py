...
M = Generic(
    ...
    SETPOINT_MEASUREMENT_PERIOD=(460, 490),
)
        ...
        if i_e > 0:
            ...
            if m.HETERO_COMP_MECH.startswith('secreted_regulation'):
                ...
                if i_e > m.SETPOINT_MEASUREMENT_PERIOD[1]:
                    ...
                    sigmoid_transform_secreted_diffs = sigmoid_tranform(secreted_diffs)

                    w_update = sigmoid_transform_secreted_diffs.reshape(sigmoid_transform_secreted_diffs.shape[0], 1) * np.ones((m.N_EXC, m.N_EXC + m.N_UVA)).astype(float)
