...
def ff_unit_func(m):
    ...
    return gaussian_if_under_val(1, (m.PROJECTION_NUM, m.PROJECTION_NUM), w, 0)
...
def run_test(m, output_dir_name, n_show_only=None, add_noise=True, dropout={'E': 0, 'I': 0},
    ...
    pairwise_spk_delays = np.block([
        [int(2.2e-3 / S.DT) * np.ones((m.N_EXC, m.N_EXC)), np.ones((m.N_EXC, m.N_UVA)), int(0.25e-3 / S.DT) * np.ones((m.N_EXC, m.N_INH))],
        [int(0.25e-3 / S.DT) * np.ones((m.N_INH + m.N_UVA, m.N_EXC + m.N_INH + m.N_UVA))],
    ]).astype(int)
