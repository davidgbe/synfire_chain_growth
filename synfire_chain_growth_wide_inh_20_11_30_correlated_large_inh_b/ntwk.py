...
from utils.general import time_stamp, zero_pad
...
class LIFNtwkG(object):
    ...
    def __init__(self, c_m, g_l, e_l, v_th, v_r, t_r, e_s, t_s, w_r, w_u, plasticity_indices, connectivity, W_max, m, eta, epsilon, dt,
    ...
    def update_w(self, t_ctr, spks, dt):
        ...
        delta_stdp = np.multiply(normed_w_r_e, (o - o.T) + np.outer(curr_spks, curr_spks))
    ...
    def run(self, dt, clamp, i_ext, spks_u=None):
        ...
        if self.output:
            output_dir = f'./data/{time_stamp(s=True)}'
