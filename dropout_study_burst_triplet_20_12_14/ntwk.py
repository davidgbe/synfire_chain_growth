...
class LIFNtwkG(object):
    ...
    def __init__(self, c_m, g_l, e_l, v_th, v_r, t_r, e_s, t_s, w_r, w_u, plasticity_indices, connectivity, W_max, m, eta,
        ...
        self.num_pl_indices = len(plasticity_indices)
        ...
        self.transpose_indices = np.arange(0, self.num_pl_indices**2).reshape(self.num_pl_indices, self.num_pl_indices).T.reshape(self.num_pl_indices**2)
    ...
    def update_w(self, t_ctr, spks, dt):
        ...
        w_r_e_plastic = self.w_r['E'][self.w_r_e_plastic_mask].reshape(self.num_pl_indices, self.num_pl_indices)

        if t_points_for_stdp > 0:
            sparse_curr_spks = csc_matrix(curr_spks)
            sparse_spks = csc_matrix(spks[:-1, :])

            spk_correlations = kron(curr_spks, sparse_spks)
            spk_correlations_for_cache = spk_correlations.tocsc()[:, self.transpose_indices]


            o = spk_correlations.T.dot(self.kernel[:t_points_for_stdp]).reshape(num_cells, num_cells)
