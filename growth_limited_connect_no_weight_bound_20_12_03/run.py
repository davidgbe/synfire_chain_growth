...
M = Generic(
    ...
    DRIVING_HZ=1, # 10 Hz lambda Poisson input to system
    N_DRIVING_CELLS=10,
    PROJECTION_NUM=5,
    W_INITIAL=0.,
...
def run_test(m, show_connectivity=True, repeats=1, n_show_only=None, add_noise=True, dropouts=[{'E': 0, 'I': 0}]):
    ...
    w_u_e_drive = np.zeros(n_cells_driven)
    w_u_e_drive[:m.PROJECTION_NUM] = m.W_U_E
    w_u_e = np.diag(w_u_e_drive)
    ...
    connectivity = np.stack([rand_n_ones_in_vec_len_l(m.CON_PER_N, m.N_EXC) for i in range(m.N_EXC)])
    ...
    for rp_idx in range(repeats):
        ...
        neuron_active_mask = np.random.rand(spks_u.shape[0], n_cells_driven) < m.INPUT_RELIABILITY
...
m2.W_MAX = 0.26 * 0.004
m2.M = 5
m2.W_INITIAL = m2.W_MAX / (m2.M * np.sqrt(m2.N_EXC))
m2.CON_PER_N = 10

for i in [.8, .9]:
    m = copy(m2)
    m.INPUT_RELIABILITY = i
    all_rsps = quick_plot(m)
