...
def generate_chain_weight_mat(m):
    ...
    w_syn *= m.W_MAX / m.CON_PER_ROW
    return w_syn

def generate_local_con(m, n_skip=[1]):
    chain_order = np.arange(1, int(m.N_EXC / m.PROJECTION_NUM), dtype=int)
    chain_order = np.insert(chain_order, 0, 0)
    rand_con = np.zeros((m.N_EXC, m.N_EXC))

    for idx, layer_idx in enumerate(chain_order):
        layer_start = m.PROJECTION_NUM * layer_idx
        rand_con[layer_start : layer_start + m.PROJECTION_NUM, layer_start : layer_start + m.PROJECTION_NUM] = m.RAND_WEIGHT_MAX * np.random.rand(m.PROJECTION_NUM, m.PROJECTION_NUM)
        
        for n in n_skip:
            if idx + n < len(chain_order):
                next_layer_idx = chain_order[idx + n]
                next_layer_start = m.PROJECTION_NUM * next_layer_idx

                rand_con[next_layer_start : next_layer_start + m.PROJECTION_NUM, layer_start : layer_start + m.PROJECTION_NUM] = m.RAND_WEIGHT_MAX * np.random.rand(m.PROJECTION_NUM, m.PROJECTION_NUM)
    return rand_con
    ...
    for rp_idx in range(repeats):
        ...
        for d_idx, dropout in enumerate(dropouts):
            ...
            rsp = ntwk.run(dt=S.DT, clamp=clamp, i_ext=i_ext,
                            ...
                            dropouts=[(m.DROPOUT_TIME, dropouts[d_idx])], m=m,
                            )
...
def quick_plot(m, run_title='', w_r_e=None, w_r_i=None, repeats=1, show_connectivity=True, n_show_only=None, add_noise=True, dropouts=[{'E': 0, 'I': 0}]):
    ...
    output_dir_activations = f'./activations/{output_dir_name}'
    os.makedirs(output_dir_activations)
    ...
    for idx_r, rsps in enumerate(all_rsps):
        for idx_do, rsp_for_dropout in enumerate(rsps):
            ...
            plot_params = {
                'early': driving_activation_times[:10],
                'late': driving_activation_times[-10:],
                'middle': driving_activation_times[driving_activation_times > int(m.DROPOUT_TIME / S.DT)][:10],
            }
            ...
            for title, trigger_times in plot_params.items():
                for t_idx, trigger_time in enumerate(trigger_times):
                    ...
                    sio.savemat(output_dir_activations + '/' + f'{title}_{t_idx}', {
                        'exc_raster': exc_raster_for_win,
                        'inh_raster': inh_raster_for_win,
                    })
    return all_rsps
...
w_r_e, w_r_i = load_weight_matrices('./data/2e-4_scale_1.5e-3_pen_2021-01-22--10:15--51:0689_0_0', 390)
